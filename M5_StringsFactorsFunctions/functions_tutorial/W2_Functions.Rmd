...
* The data used in this exercise is the Global Mammals Parasite Database (GMPD)
...
* Open the data files, match the column titles to the metadata explanation  
...
```{r}
library(tidyverse)
gmp <- read_csv("GMPD_data/GMPD_main.csv")
...
2. What arguments does the function need.

The arguments are what is fed into the function. In this case, we need to tell the function which dataset to feed into the analysis pipeline. 
...
*Step 4.* Try it out
  Once you've wrote your function, you'll need to load it into the environment by executing the code. Then you can call that analysis just like the other functions we've been using!
...
2. Make some comments at the start of the file, write your functions
    - *Note:* There is some bad advice about scripts you might come across. Never start a script by cleaning the environment (`rm(list = ls())`) or by setting the working director (`setwd()`)
...
For the sake of this demonstration, let's start with a fresh session of `R` by clicking on Session > Restart R. You'll notice the Environment tab is now empty. 
...
Is the `PrevByHostGroup()` function in your environment? What is returned when you type `PrevByHostGroup` into the console? What about `PrevByHostGroup()`? 
